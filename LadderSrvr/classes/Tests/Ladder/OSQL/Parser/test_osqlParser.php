...
class test_osqlParser extends \Tests\Test
{
	protected $nLoops = 1;
	...
	(
		...
		(
			"
				SELECT INSTANCES
				USING CLASS Common_Todo [a]
				FROM '\RootFolder\Todos' [b]
				WHERE
					a = ( 5 + 1 )
			",
			"54, 55, 58, 46, 4, 0, 59, 6, 1, 60, 4, 2, 73, 4, 0, 1",
		...
		(
			"
				SELECT FIELDS a.dCreated, a.bComplete, a.szMemo
				USING CLASSES Common_Todo [a]
				FROM '\RootFolder\Todos' [b]
				WHERE
					b CONTAINS a AND
					a.dTarget BETWEEN #2015-01-20# AND #2015-01-21# AND
					NOT a.bCompleted 
				ORDER BY
					a.szTaskType, a.nPriority
			",
			"54, 55, 58, 46, 4, 0, 59, 6, 1, 60, 4, 2, 73, 4, 0, 1",
			array ("Common_Todo"), 
		),
	...
	function run ()
	{
		...
		$szOSQL = $this->aryStatements [212][0];

		$objParser = new osqlParser ();

		$this->assert ($objParser->process ($szOSQL), "The string was parsed successfully");
		$this->log ("OSQL", $szOSQL);
		$this->log ("ByteCode", $objParser->parser->scanner->ByteCode()->toString());
		
		
		print ("<pre>");
		print_r ($objParser->stateData);
		print ("<P>");
